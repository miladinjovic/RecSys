...
Skriptom **BayesianPersonalizedRanking.py** se ispisuje prosečna vrednost AUC metrike, pošto je algoritam BayesianPersonalizedRanking u suštini klasifikator (određuje da li će se korisniku dopasti određeni proizvod ili ne ). Takođe, pokretanjem ove skripte generišu se i standardne metrike top-n evaluacije ovog algoritma (hit rate, cumulative hit rate, average reciprocal hit rate itd). Naravno, što su ove vrednosti veće od nule, to je lista koju algoritam generiše "kvalitetnija". Parametre algoritma možete menjati izmenom vrednosti u promenljivi bpr_params. 
...
## Napomena
U folderu **Slike** smešteni su dijagrami dobijeni pokretanjem skripte exploration.py, kao i skrinšotovi rezultata top-n evaluacije različitih algoritama. Datoteka **requirements.txt** sadrži spisak svih potrebnih biblioteka, koji će vam pomoći da lako kreirate venv na vašem računaru.
